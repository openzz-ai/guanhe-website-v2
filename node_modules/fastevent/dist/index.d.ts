...
type ClosestMatch<T> = UnionToTuple$1<T> extends infer Tuple extends any[] ? Tuple[IndexOfMin<ToKeyPrioritys<Tuple>>] : never;
...
declare class FastEvent<Events extends Record<string, any> = Record<string, any>, Meta extends Record<string, any> = Record<string, any>, Context = never, AllEvents extends Record<string, any> = Expand<Events & FastEvents>, Types extends keyof AllEvents = KeyOf<AllEvents>, AllMeta extends Record<string, any> = FastEventMeta & Meta, EventNames = KeyOf<ExtendWildcardEvents<AllEvents>>> {
    ...
    * @example
     ...
     */
    broadcast<R = any, T extends string = string>(type: ReplaceWildcard<T> | Types, payload?: InMatchedEvent<Events, T> extends true ? GetPayload<UnTransformedEvents<AllEvents>, T> : any, callback?: (type: string, message: FastEventMessage, args: FastEventListenerArgs<any>) => [FastEventMessage, FastEventListenerArgs<any>] | FastEventMessage | null, retain?: boolean): R[];
    /**
     * 异步触发事件
     * @param type - 事件类型，可以是字符串或预定义的事件类型
     * @param payload - 事件数据负载
